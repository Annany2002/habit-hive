...
function Reminder({
  ...
  habitItem: HabitType;
  setHabitItem: Dispatch<SetStateAction<HabitType>>;
}) {
  ...
  const toggleUpdate = () => {
    const copyHabitItem = { ...habitItem };
    copyHabitItem.isNotificationOn = !notificationOn;
    setHabitItem(copyHabitItem);
    ...
    <div className="flex flex-col gap-2 p-3 mt-2">
      ...
      {showTimer && (
        <div
          ...
          className="flex justify-between p-2 m-2 mt-4 rounded-md"
        >
          <span style={{ color: color.myGreen }}>Select Time</span>
          <div
            className="flex gap-2 items-center justify-center cursor-pointer select-none"
            onClick={() => setOpenTimePickerWindow(true)}
          >
            <span style={{ color: color.myGreen }} className="tracking-wide">
              {habitItem.notificationTime === ""
                ? "none"
                : habitItem.notificationTime}
            </span>
            <ArrowDown color={color.myGreen} />
          </div>
        </div>
      )}
    </div>
